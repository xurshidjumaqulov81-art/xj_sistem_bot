...
async def init(dsn: str):
    ...
    async with _pool.acquire() as conn:
        ...
        CREATE TABLE IF NOT EXISTS users (
            ...
            stage2_text_done BOOLEAN DEFAULT FALSE,
            stage2_audio_done BOOLEAN DEFAULT FALSE,
            stage2_video_done BOOLEAN DEFAULT FALSE,
            stage2_links_done BOOLEAN DEFAULT FALSE,

            stage3_idx INT DEFAULT 0,
            stage3_waiting BOOLEAN DEFAULT FALSE,
...
async def ensure_user(user_id: int, inviter_id: int | None = None):
    ...
    async with pool.acquire() as conn:
        ...
        if row:
            # inviter_id bo'sh bo'lsa, set qilamiz
...
async def mark_stage2(user_id: int, key: str):
    # key: text_done/audio_done/video_done/links_done
    mapping = {
...
async def is_stage3_waiting(user_id: int) -> bool:
    pool = _p()
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT stage3_waiting FROM users WHERE user_id=$1", user_id)
        return bool(row["stage3_waiting"]) if row else False
...
async def save_stage3_note(user_id: int, idx: int, note: str):
    ...
    async with pool.acquire() as conn:
        await conn.execute("""
            ...
            VALUES($1,$2,$3)
            ON CONFLICT(user_id, idx) DO UPDATE SET note=EXCLUDED.note, created_at=NOW()
        """, user_id, idx, note)
